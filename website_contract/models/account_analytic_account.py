...
class account_analytic_account(models.Model):
    ...
    def remove_option(self, option_id):
        opt_line = self.env['account.analytic.invoice.line.option'].browse(option_id)
    ...
    def change_subscription(self, new_template_id):
        ...
        for line in new_template.recurring_invoice_line_ids:
            if line.product_id not in rec_lines_to_modify and line.product_id not in [cur_line.product_id for cur_line in self.recurring_invoice_line_ids]:
    ...
    def _do_payment(self, payment_method, invoice, two_steps_sec=True):
        ...
        values = {
            ...
            's2s_cb_eval': "self.env['account.analytic.account'].reconcile_pending_transaction(%s,self,self.invoice_id)" % self.id
        }
        ...
        for line in self:
            for opt_line in line.analytic_account_id.option_invoice_line_ids:
                if line.product_id == opt_line.product_id and line.id != opt_line.id:
                    raise ValidationError("You cannot use the same product as an option twice for the same contract template.")

    @api.depends('portal_access')
    def _compute_is_authorized(self):
        for option in self:
            option.is_authorized = bool(self.env['account.analytic.account'].search_count([('template_id', '=', option.analytic_account_id.id)]))
