...
var DataExport = Dialog.extend({
    ...
    start: function() {
        ...
        this.$('#import_compat').change(function() {
            self.$('#fields_list').empty();
            self.$('#field-tree-structure').remove();
            var import_comp = self.$("#import_compat").val();
    ...
    do_load_export_field: function(field_list) {
        var export_node = this.$("#fields_list");
    ...
    on_show_domain: function() {
        this.$('tr').first().find('td').append(QWeb.render('ExportTreeView-Domain', {'record': this}));
    },
    on_show_data: function(result, after) {
        var self = this;

        if (after) {
            var current_tr = self.$("tr[id='treerow-" + after + "']");
            ...
            self.$('#left_field_panel').append(QWeb.render('ExportTreeView-Secondary', {'fields': result}));
        }
        _.each(result, function(record) {
            self.records[record.id] = record.value;
            if (record.required) {
                var required_fld = self.$("tr[id='treerow-" + record.id + "']").find('#tree-column');
                required_fld.addClass("oe_export_requiredfield");
            }
            self.$("img[id='parentimg-" + record.id +"']").click(function() {
                self.on_click(this.id, record);
            });

            self.$("tr[id='treerow-" + record.id + "']").click(function(e) {
                if (e.shiftKey) {
                    var frst_click, scnd_click = '';
                    if (self.row_index === 0) {
                        self.row_index = this.rowIndex;
                        frst_click = self.$("tr[id^='treerow-']")[self.row_index-1];
                        $(frst_click).addClass("ui-selected");
                    } else {
                        var i;
                        if (this.rowIndex >=self.row_index) {
                            for (i = (self.row_index-1); i < this.rowIndex; i++) {
                                scnd_click = self.$("tr[id^='treerow-']")[i];
                ...
                self.$("tr[id='treerow-" + record.id + "']").keyup(function() {
                    self.row_index = 0;
                });
                var o2m_selection = self.$("tr[id='treerow-" + record.id + "']").find('#tree-column');
            ...
            self.$("tr[id='treerow-" + record.id + "']").keydown(function(e) {
            ...
            self.$("tr[id='treerow-" + record.id + "']").dblclick(function() {
                var $o2m_selection = self.$("tr[id^='treerow-" + record.id + "']").find('#tree-column');
                if (!$o2m_selection.hasClass("oe_export_readonlyfield")) {
        ...
        self.$('#fields_list').mouseover(function(event) {
            if (event.relatedTarget) {
                if (event.relatedTarget.attributes['id'] && event.relatedTarget.attributes['string']) {
                    ...
                    if (field_id && field_id.split("-")[0] === 'export') {
                        if (!self.$("tr[id='treerow-" + field_id.split("-")[1] + "']").find('#tree-column').hasClass("oe_export_readonlyfield")) {
    ...
    showcontent: function(id) {
        ...
        var $this = this.$("tr[id='treerow-" + id + "']");
        ...
        var child_field = this.$("tr[id^='treerow-" + id +"/']");
    ...
    add_field: function(field_id, string) {
        var field_list = this.$('#fields_list');
        if (this.$("#fields_list option[value='" + field_id + "']")
                && !this.$("#fields_list option[value='" + field_id + "']").length) {
    ...
    on_click_export_data: function() {
        var self = this;
        var exported_fields = this.$('#fields_list option').map(function () {
        ...
        var export_format = this.$("#export_format").val();
        ...
        this.session.get_file({
            ...
            data: {data: JSON.stringify({
                ...
                import_compat: !!this.$("#import_compat").val(),
            })},
